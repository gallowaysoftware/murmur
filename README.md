...
- **Single-goroutine streaming runtime.** Phase-1 streaming processes records sequentially per worker. Throughput ceiling is *estimated* at roughly 5–10 k events/s/worker depending on item size — this is a design-time figure, **not a measurement**: no benchmark in this repo runs against DynamoDB, local or real (see `doc/design.md` §15). Measured numbers are a deliverable of the v1 soak. Use [`streaming.WithConcurrency(N)`](pkg/exec/streaming) for in-process key-hash fanout, or scale horizontally with Kafka partitions. For decode-bound workloads (heavy Protobuf, encrypted payloads), [`kafka.Config.Concurrency`](pkg/source/kafka/source.go) parallelizes the source-side decode with per-partition order preserved.
