...
1. **Sketch portability across runtimes.** Valkey HLL uses Redis's encoding; Spark uses its own; pure-Go libraries vary. We need bit-compatibility OR explicit conversion at runtime boundaries. Decision sketch: standardize on a portable encoding (HLL++ / Druid layout) for state stored in DDB; backend-native encodings only inside live workers; conversion at the Valkey-cache and Spark-output boundaries.
2. **Worker autoscaling signal.** Kinesis: shard count. Kafka: consumer lag. ECS Fargate doesn't autoscale on these natively — need a custom CloudWatch metric + target tracking, or a sidecar that emits the signal. Probably solve in Phase 1 since autoscaling matters for streaming workers.
3. **Bootstrap-to-live handoff watermarking.** When Bootstrap finishes, the live stream needs to start from the right position to avoid gaps or double-count. For Mongo: capture the change-stream resume token at snapshot-start, switch to streaming from that token after bootstrap completes (Debezium pattern). For DDB: capture the DDB Streams shard iterator position at snapshot-start, similarly. Per-source-type concern; document in `bootstrap-and-replay.md`.
4. **Window bucket-merge cost at query time.** A "last 30 days" query on a daily-windowed pipeline reads 30 DDB items and merges. Cheap for Sum (30 numbers); not free for HLL (30 sketch merges). Mitigations to consider: pre-rolled "last-7-days" / "last-30-days" rollup buckets maintained alongside daily; Valkey-cached pre-merged windows; lazy-rollup on first query. Probably defer to Phase 2 unless query latency is unacceptable in MVP benchmarks.
5. **Window boundary semantics under replay.** When we replay events into a fresh state table, time-bucket assignment is based on the event timestamp, not the processing time. Need to make sure replay drivers respect event-time bucketing — and that streaming workers have a config for whether to bucket by event-time or processing-time. Default event-time, fall back to processing-time when timestamps are missing.
