...
| `pkg/query` | mostly stable | `Get` / `GetWindow` / `GetRange` / `LambdaQuery` are likely v1 surface. Windowed reads validate the request before fanning out: non-positive durations, swapped / unset / unrepresentable range bounds, windows longer than `Retention`, and bucket spans past `windowed.Config.MaxBuckets` all return an error matching `query.ErrInvalidQuery` instead of a monoid Identity that looks like a real zero |
