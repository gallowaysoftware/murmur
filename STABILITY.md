...
| `pkg/monoid/sketch/{hll,topk,bloom}` | experimental | On a decode error `Combine` returns the operand that decoded and discards the other — unavoidable while `Monoid.Combine` has no error return, but now reportable via `WithDecodeErrorHandler`; wire it to a `metrics.Recorder` or the loss stays invisible. The same hook reports **parameter mismatches**: a `bloom` shape clash (two filters whose (m, k) differ cannot be OR'd — `Combine` keeps the left operand) and a `topk` K clash (`Combine` merges at the widest K it can see rather than truncating the wider summary). `bloom.Bloom` / `bloom.NewWithCapacity` Identity is now the empty slice, so it is an identity for an operand of ANY shape. `topk.Inspect` returns a `Summary` (`Ingested` / `Retained` / `Coverage` / `Saturated` / `MaxError` / `PartialWeight`) — experimental, and the only way to tell a saturated sketch from an exact one. The topk wire format carries the ingested weight in a flagged header; pre-flag rows still decode, with `PartialWeight` set. Cross-runtime encoding portability not yet proven |
| `pkg/monoid/compose` | mostly stable | `MapMerge` / `Tuple2` / `DecayedSum`; FP-associativity caveats apply to `DecayedSum`. `DecayedSum` / `DecayedSumBytes` take experimental options: `WithDecodeErrorHandler` (bytes only — a non-17-byte operand is now an error, not a value assembled from someone else's blob), `WithClockSkewBound` (default two half-lives; bounds how far ahead of the wall clock an observation may set the reference frame, so one future-dated event cannot freeze a key) and `WithClock`. `DecodeDecayed` returns `(Decayed, error)`. A non-positive half-life means "no decay" in `Combine` as well as `EvaluateAt`, and `EvaluateAt` before the reference time returns the stored value rather than scaling it up |
