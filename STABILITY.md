...
| `pkg/query/typed` | experimental | typed-client wrappers over the generic QueryService — `SumClient`, `HLLClient`, `TopKClient`, `BloomClient`. All four expose `Get` and `GetWindow`; all four also expose `GetWindowMany` for batched windowed reads (returns parallel arrays — missing entities yield zero-valued results). `SumClient` additionally has `GetMany` and `GetRange`. The decoders + typed shape behind application-service typed-wrapper RPCs (see `examples/typed-wrapper`). Building block under `cmd/murmur-codegen-typed` |
