...
func TestDeployed_PageViewCounters_RoundTrip(t *testing.T) {
	...
	// --- Produce records FIRST, before starting the worker. ---
	//
	// Why this order: franz-go's consumer-group join assigns
	// partitions based on the topic's current metadata. If the topic
	// doesn't exist when the worker joins (because no producer has
	// touched it yet), kgo gets a 0-partition assignment and waits
	// for MetadataMaxAge (5 min default) before rediscovering. The
	// kafka-console-consumer probe doesn't hit this because the
	// probe test produces first too. Auto-create-topics is enabled
	// on the broker, so the producer's first ProduceSync creates
	// the topic; the worker then joins to a topic that already
	// exists.
	...
	t.Logf("produced 9 records across 3 keys to %s before starting worker", topic)
	...
	t.Logf("polling %s for results", queryURL)
...
// yet match `want`.
//
// Uses GetWindow — not Get — because the page-view-counters pipeline
// is Daily-windowed (writes land in today's bucket, not bucket 0).
// QueryService.Get reads bucket 0 only. QueryService.GetWindow with
// a 1-day duration merges today's bucket and gives the right total.
func queryAllPagesGRPCConnect(t *testing.T, ctx context.Context, httpc *http.Client, baseURL string, want map[string]int64) (map[string]int64, int) {
	...
	for entity, expected := range want {
		body := strings.NewReader(fmt.Sprintf(`{"entity":%q,"duration_seconds":86400}`, entity))
		req, _ := http.NewRequestWithContext(ctx, http.MethodPost,
			baseURL+"/murmur.v1.QueryService/GetWindow", body)
		...
		// GetWindow envelope: { "value": { "data": "<base64>" } } (no
		// per-entity "present" — windowed reads return zero-merged
		// values for missing entities).
		var env struct {
			Value struct {
				Data []byte `json:"data"`
			} `json:"value"`
...
//
// Reduced-scope companion to the full round-trip test — narrower
// failure-mode coverage (Dockerfile mis-builds, env-var schema
// drift, the query binary not starting against real DDB-local, the
// Connect-RPC handler not responding) without paying the worker
// container's build + kafka-spinup cost.
